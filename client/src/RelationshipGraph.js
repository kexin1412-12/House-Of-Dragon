...
// Graph data + portraits ship as static assets in client/public so the Vercel
// build renders without a backend. Snapshots are regenerated by
// server/scripts/build-graph-snapshots.js. portrait_url paths are same-origin
// (start with `/kb/...`), so the empty API base is intentional.
const API = '';
...
export default function RelationshipGraph({ videoId, defaultHero = 'rhaenyra_targaryen' }) {
  ...
  // 拉数据：发布版没有后端，从 client/public/relationship-graph/<id>.json 读快照
  const fetchGraph = useCallback(async (heroId) => {
    ...
    try {
      const { data: resp } = await axios.get(
        `${SNAPSHOT_BASE}/relationship-graph/${encodeURIComponent(heroId)}.json`,
        { timeout: 8000 },
      );
