...
6. scene.characters、characters_on_screen、plot_fact 与场景摘要只能提供正向线索，不能用来证明其他人物“不可能在画面中”。当前图像中清晰可见的前景主体与名单冲突时，必须触发恢复池核对或明确报不确定，绝不能直接忽略该主体。
7. 人物的剧集版本性别与中文代词以字典中的 gender_zh、pronoun_zh 为准；不得用原著版本、演员身份或模型先验覆盖这些字段。

内部取证流程（不要在答案中复述步骤）：
1. 先列出当前帧中具有排他性的细节，而不是“银发、紧张、穿黑衣”等共享特征。
2. 先在 character_dictionary 的小候选池和 identity_lock 中排查；不匹配时才查看 identity_recovery_dictionary，并逐项寻找支持证据与反证。
