...
function register(app) {
  ...
  // ─── 集内章节：进度条 ticks 用 ──────────────────────────────
  // 把这一集 KB 里的 ~711 个原子 scene 压成几个面向用户的 chapter，
  // 让进度条上能看到"现在在哪一段"。两类节点：
  //   · branch_point —— 真正的叙事分支锚点（彩色 tick），全部带出
  //   · act marker  —— 把全片切成 ~8 等份，每份用最靠近的 scene 的
  //                    plot.fact 当 label（灰色 tick）
  // 都用秒数作 timestamp，前端按 t/duration 算百分比放 tick。
  app.get('/api/agent/timeline/chapters', (req, res) => {
    const { videoId } = req.query;
    if (!videoId) return res.status(400).json({ error: 'videoId required', chapters: [] });
    const kb = loadKB(videoId);
    if (!kb) return res.json({ chapters: [], duration: 0, has_kb: false });

    const scenes = (kb.scenes || []).filter(s => Number.isFinite(s.start_time));
    const duration = scenes.length
      ? Math.max(...scenes.map(s => Number(s.end_time) || 0))
      : Number(kb.duration) || 0;
    ...
    // act markers: 8 等分，每个分位点取最近的一个 scene
    const ACT_COUNT = 8;
    const acts = [];
    if (scenes.length && duration > 0) {
      for (let i = 0; i < ACT_COUNT; i++) {
        const target = (duration / ACT_COUNT) * i;
        let best = scenes[0];
        let bestDist = Math.abs(scenes[0].start_time - target);
        for (const s of scenes) {
          const d = Math.abs(s.start_time - target);
          if (d < bestDist) { best = s; bestDist = d; }
        }
        // 同一 scene 不重复（尾部 act 可能落到同一个 scene）
        if (acts.length && acts[acts.length - 1].scene_id === best.scene_id) continue;
        const fact = best.plot?.fact || best.plot?.reading || best.narrative || '';
        acts.push({
          kind: 'act',
          t: Number(best.start_time) || 0,
          scene_id: best.scene_id,
          label: truncate(fact, 28) || (best.tags && best.tags[0]) || `场景 ${best.scene_id}`,
        });
      }
    }

    // branch_points：直接带出，前端涂彩色高亮
    const branches = (kb.branch_points || [])
      .filter(b => Number.isFinite(b.timestamp))
      .map(b => ({
        kind: 'branch',
        t: Number(b.timestamp),
        branch_id: b.branch_id,
        label: b.label || b.title || '剧情分支',
        decision_holder_display: b.decision_holder_display || null,
      }));

    // 合并 + 按 timestamp 排序；branch 优先级高于 act（同 t 时保留 branch）
    const merged = [...acts, ...branches].sort((a, b) => a.t - b.t);
    const dedup = [];
    for (const c of merged) {
      const last = dedup[dedup.length - 1];
      if (last && Math.abs(last.t - c.t) < 5) {
        if (c.kind === 'branch') dedup[dedup.length - 1] = c; // branch 覆盖 act
      } else {
        dedup.push(c);
      }
    }
