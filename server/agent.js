...
- 语气可以结合龙之家族和权力的游戏风格，但必须要自然，让观众有沉浸感
...
同时如果有必要，你可以提醒观众这段情节需要重点关注的地方，比如某个物品的意义、某个角色的表情、某句台词的双关。
...
但是所有的解读都是用来解释剧情的发展和人物关系的，而不是为了展示解读本身。不要为了显得专业硬塞解读角度，只有当它们能真正帮助用户理解当前这一刻时才用。
═══ 判断要不要用的标准 ═══
...
═══ 长度 ═══
默认 2-10，最多 100
只有用户明确问"详细讲"，才可以稍微展开。也可以在结尾问观众需要详细展开吗
不要结尾加"如果你想……"这种话。`;
...
═══ 输出格式（严格） ═══
全文 3-4 段。
前面 2-3 段是叙事性的场景 + 心理描写（每段 60-100 字），用平静克制的语气写，不要堆华丽辞藻。
最后一段必须**另起一行**单独成段，是 1-2 个开放式问句（每问 30-60 字）。问句之间也用空行分隔。不要陈述句、不要总结、不要给暗示性的答案。
**全文严禁出现任何 emoji 或符号前缀**（不要 💭 ⚠️ 🔀 ⏵ ✦ ➜ → ← 这类）。问句段直接以问句开始。
段与段之间统一用一个空行分隔。
全文严格控制在 380 字以内（中文计）。`;
...
请写"如果你的选择真的发生了"那个世界线。前面 2-3 段照常推演，最后一段抛 1-2 个开放问句给观众，让他们继续想。`;
  ...
  // ─── 多视角同场景重写 / Multi-POV Scene Rewrite ───────────────────────
  // POST /api/agent/scene/povs
  // body: { videoId, startTime, endTime, sceneLabel?, characterIds? (≤4) }
  // 返回流式 SSE：先 meta（角色身份卡），再为每个角色顺序流式输出一段第一人称
  // 内心散文（Martin POV 章节风），最后 done。
  // 前端按 [POV:character_id] 锚点切分到 3 个独立栏。
  app.post('/api/agent/scene/povs', async (req, res) => {
    const { videoId, startTime, endTime, sceneLabel, characterIds } = req.body || {};
    res.setHeader('Content-Type', 'text/event-stream; charset=utf-8');
    res.setHeader('Cache-Control', 'no-cache, no-transform');
    res.setHeader('Connection', 'keep-alive');
    res.setHeader('X-Accel-Buffering', 'no');
    ...
    const startT = Number(startTime);
    const endT = Number(endTime);
    if (!videoId || !Number.isFinite(startT) || !Number.isFinite(endT) || endT <= startT) {
      send('text', { delta: '缺少 videoId 或时间区间无效。' });
      send('done', { source: 'error' });
      return res.end();
    }
    const kb = loadKB(videoId);
    ...
    // 1. 找区间内所有 scene + 聚合 characters by 总 screen_time
    const overlap = kb.scenes.filter(s => s.end_time > startT && s.start_time < endT);
    if (overlap.length === 0) {
      send('text', { delta: '这段时间没有可识别的场景。' });
      send('done', { source: 'error' });
      return res.end();
    }
    const charScore = new Map(); // id → screen_time_s 累计
    for (const sc of overlap) {
      for (const c of (sc.characters || [])) {
        if (!c.id) continue;
        charScore.set(c.id, (charScore.get(c.id) || 0) + (Number(c.screen_time_s) || 0));
      }
    }
    let pickedIds;
    if (Array.isArray(characterIds) && characterIds.length > 0) {
      pickedIds = characterIds.slice(0, 4).filter(id => charScore.has(id));
    } else {
      pickedIds = [...charScore.entries()]
        .sort((a, b) => b[1] - a[1])
        .slice(0, 3)
        .map(([id]) => id);
    }
    if (pickedIds.length === 0) {
      send('text', { delta: '场景里没有匹配到可重写的角色。' });
      send('done', { source: 'error' });
      return res.end();
    }

    // 2. 取每个角色的 profile（cursor = endTime → 用 episode 边界做 spoiler safety）
    const showId = kb.show_id || 'house-of-the-dragon';
    const episode = resolveEpisode(kb) || '';
    const db = getCharacterDb(showId);
    const cursor = episode || null;
    const povs = [];
    for (const id of pickedIds) {
      const card = charactersLib.lookupCharacter(db, id, cursor);
      const rp = charactersLib.lookupRoleplayProfile(showId, id, episode);
      if (!card && !rp) continue;
      povs.push({
        character_id: id,
        display_name: rp?.display_name_zh || card?.display_name || id,
        short_identity: rp?.short_identity_zh || card?.short_identity || card?.current?.title || '',
        portrait: card?.current?.portrait || card?.default_portrait || null,
        traits: rp?.core_traits_zh || [],
        speech: rp?.speech_pattern_zh || '',
        voice: rp?.voice_zh || '',
      });
    }
    if (povs.length === 0) {
      send('text', { delta: '场景角色还没有可用的人物档案。' });
      send('done', { source: 'error' });
      return res.end();
    }

    // 3. scene 上下文：用首个 scene 的 fact / reading 作为锚（区间通常对应 1-3 个 scene）
    const anchorScene = overlap[0];
    const fact = overlap.map(s => s.plot?.fact).filter(Boolean).join(' ');
    const reading = overlap.map(s => s.plot?.reading).filter(Boolean).slice(0, 2).join(' ');
    const fmtTs = (t) => `${Math.floor(t / 60)}:${String(Math.floor(t % 60)).padStart(2, '0')}`;
    const tsLabel = `${fmtTs(startT)}-${fmtTs(endT)}`;

    // 4. 字幕窗：endT 是上界，避免暴露后续台词
    const cues = srtWindow(videoId, (startT + endT) / 2, (endT - startT) / 2 + 5, 0)
      .filter(c => c.start >= startT - 2 && c.start <= endT + 2);
    const subtitleBlock = cues.length
      ? cues.map(c => `${fmtTs(c.start)} | ${c.text}`).join('\n')
      : '（这段没有清晰对白。）';
    ...
    // 5. 先发 meta
    send('meta', {
      scene_label: sceneLabel || anchorScene.label_zh || anchorScene.scene_id,
      time_label: tsLabel,
      povs: povs.map(p => ({
        character_id: p.character_id,
        display_name: p.display_name,
        short_identity: p.short_identity,
        portrait: p.portrait,
      })),
    });

    // 6. 一次 LLM 调用，让它按 [POV:id] 锚点把 N 段连写
    const showName = kb?.show_id === 'house-of-the-dragon' ? '《龙之家族》' : `《${kb?.title || videoId}》`;
    const povBlock = povs.map((p, i) =>
      `${i + 1}. [POV:${p.character_id}] · ${p.display_name}（${p.short_identity}）
   核心特质：${(p.traits || []).join('、') || '—'}
   说话方式：${p.speech || '—'}
   声音质感：${p.voice || '—'}`
    ).join('\n');

    const system = `你是 HBO ${showName} 的编剧。任务：把同一个场景从 ${povs.length} 个不同角色的视角，**各**写一遍内心化散文。

═══ 形式 ═══
每个 POV 都用马丁 POV 章节的语气：第一人称内心独白 + 短克制的动作叙述穿插。冷静、有重量、不堆华丽辞藻。
每段以单行锚点开头，**这一行必须严格匹配格式**：
[POV:character_id]
（id 用下面给的英文 id，不要翻译）
然后空一行，再写 80-130 字的内心 + 动作散文。
所有 POV 写完后停笔，不要总结、不要结尾点题。

═══ 硬性原则 ═══
1. 同一个场景，三个人看到的事实应该有重叠也有差异 —— 谁注意到了什么、谁没看到什么、谁误解了什么。
2. 每个角色严格符合人格：戴蒙的语气≠雷妮拉的语气≠亚莉森的语气。直接读语气就该认得是谁。
3. 不要让角色在内心里讲他做了什么 —— 让他在内心里讲他想什么、他怕什么、他没说出口什么。
4. 不剧透角色还不知道的未来。
5. 直接进入第一段 [POV:id]，不要前言、不要"以下是..."。`;

    const user = `【剧集】${episode} · ${tsLabel}
【场景】${sceneLabel || anchorScene.label_zh || ''}
【发生了什么（叙事事实）】${fact}
${reading ? `【场外解读（参考，不要直抄）】${reading}` : ''}
...
请按上面顺序输出 ${povs.length} 个 POV，每个以 [POV:id] 锚点单独成行起头，再空一行写散文。`;
    ...
    try {
      ...
      if (controller.signal.aborted) return res.end();
      console.error('[scene/povs] LLM error:', err?.message || err);
      send('text', { delta: `\n\n（重写中断：${err?.message || '未知错误'}）` });
      send('done', { source: 'error' });
      res.end();
    }
  });
